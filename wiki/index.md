...
`imports/normalized/`. NOT a substitute for `knowledge-base/` (timeless reference) or
`project-context/` (behavioral memory) or `imports/normalized/` (the immutable
transcript). See `rules/wiki-rules.md`.

Status vocabulary: `Active` (current, has content) | `Stale` (flagged by `/wiki-lint`,
needs a pass) | `Orphan` (no ingest activity in several cycles — archival candidate).
